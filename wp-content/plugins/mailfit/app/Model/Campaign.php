...
class Campaign extends Model
{
        ...
        try {
            ...
            foreach ($subscribers as $subscriber) {
                if ($this->user->overQuota()) {
                    throw new \Exception("User has reached sending limit");
                }
    ...
    public function tagMessage($message, $subscriber, $msgId)
    {
        ...
        if (is_object($this->defaultMailList)) {
            $tags['CONTACT_NAME'] = $this->defaultMailList->contact->company;
            $tags['CONTACT_STATE'] = $this->defaultMailList->contact->state;
            $tags['CONTACT_CITY'] = $this->defaultMailList->contact->city;
            $tags['CONTACT_ADDRESS_1'] = $this->defaultMailList->contact->address_1;
            $tags['CONTACT_ADDRESS_2'] = $this->defaultMailList->contact->address_2;
            $tags['CONTACT_PHONE'] = $this->defaultMailList->contact->phone;
            $tags['CONTACT_URL'] = $this->defaultMailList->contact->url;
            $tags['CONTACT_EMAIL'] = $this->defaultMailList->contact->email;
            
            if (is_object($this->defaultMailList->contact->country)) {
                $tags['CONTACT_COUNTRY'] = $this->defaultMailList->contact->country->name;
            }
        }
    ...
    public function getPendingSubscribers()
    {
        ...
        $unique = $subscribers->unique(function ($r) {
            return $r['email'];
        });
    ...
    public function prepareEmail($subscriber, $server)
    {
        ...
        if ($server->allowCustomReturnPath() && is_object($server->bounceHandler)) {
            MailLog::info('Bounce handler set!');
        ...
        if($this->isAuto()) {
            ...
            foreach($this->listsSegments as $lists_segment) {
                if(!empty($lists_segment->segment_id)) {
                    $conditions[] = '(' . $lists_segment->segment->getSubscribersConditions() . ')';
                } else {
