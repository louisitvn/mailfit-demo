...
@section('content')
				...
				<br />	
				<h4 class="text-semibold mt-10">{{ trans('messages.api_docs') }}</h4>
                    
                <code style="font-size: 18px;">
                    <a href="{{ action("Controller@docsApiV1") }}">{{ action("Controller@docsApiV1") }} <i class="icon-link"></i></a></code>
                    
                <br />
				<h4 class="text-semibold mt-10">{{ trans('messages.your_api_token') }}</h4>
                    
                <code style="font-size: 18px;">
                    {{ Auth::user()->api_token }}</code>
				<br />
				<p class="alert alert-info mt-10">{!! trans('messages.api_token_guide', ["link" => action("Api\MailListController@index", ["api_token" => "YOUR_API_TOKEN"])]) !!}</p>
