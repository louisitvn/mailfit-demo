...
$(function() {
    ...
    $('.switchery').each(function() {
        if($(this).attr("data-switchery") != "true") {
            var switchery = new Switchery(this, {color: '#0073aa'});
        }
    }); 
    ...
    // pickadate mask
    $(document).on('change', '.pickadate-control', function() {      
    ...
    $('.pickadate-control').trigger('change');
    
    // Auto ecent latest post check all event
    $(document).on('change', '.checkboxes-with-check-all-first input', function() {      
        var value = $(this).attr('value');
        var box = $(this).parents('.checkboxes-with-check-all-first');
        
        if($(this).is(':checked')) {
            if(value == 'all') {
                box.find('input').prop('checked', true);
                box.find('input[value!="all"]').prop('disabled', true);
                $.uniform.update();
            }
        } else {
            if(value == 'all') {
                box.find('input').prop('checked', false);
                box.find('input[value!="all"]').prop('disabled', false);
                
                if(box.find('input').length <= 1) {
                    box.find('input[value="all"]').prop('checked', true);
                }
                
                $.uniform.update();
            } else {
                box.find('input[value="all"]').prop('checked', false);
                $.uniform.update();
            }
        }
    });
    setTimeout(function() {$('.checkboxes-with-check-all-first input:checked').trigger('change'); }, 100);
