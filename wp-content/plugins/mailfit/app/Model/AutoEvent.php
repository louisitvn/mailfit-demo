...
class AutoEvent extends Model
{
	...
	const TYPE_LATEST_POST = 'latest-post';
    ...
    public function rules($request=NULL)
    {
        ...
        switch ($this->event_type) {
			...
			case AutoEvent::TYPE_CUSTOM_CRITERIA:
				...
				break;
			case AutoEvent::TYPE_LATEST_POST:
				$rules = [
					'categories' => 'required',
					'tags' => 'required'
				];
    ...
    {
        return [
			// ['value' => self::TYPE_LATEST_POST, 'text' => trans('messages.send_every_time_new_post_published')],
		...
		switch ($this->event_type) {
			case self::TYPE_SPECIFIC_DATETIME:
				...
				if(!empty($params["specific_day"]) && !empty($params["specific_time"])) {
					$send_at = \Acelle\Library\Tool::systemTimeFromString($params["specific_day"].' '.$params["specific_time"], \Auth::user()->getTimezone());
					$this->updateData([
			...
			case self::TYPE_WEEKLY_RECURRING:				
				...
				if(!empty($params["weekly_recurring_time"])) {
					$send_at = \Acelle\Library\Tool::systemTimeFromString('2000-01-01 '.$params["weekly_recurring_time"], \Auth::user()->getTimezone());
					$this->updateData([
			...
			case self::TYPE_MONTHLY_RECURRING:
				...
				if(!empty($params["monthly_recurring_time"])) {
					$send_at = \Acelle\Library\Tool::systemTimeFromString('2000-01-01 '.$params["monthly_recurring_time"], \Auth::user()->getTimezone());
					$this->updateData([
						'time' => $send_at->toAtomString()
					]);
				}
    ...
    public function displayMessage()
    {
		...
		switch ($this->event_type) {
			...
			case self::TYPE_API_CALL:				
				...
				break;
			case self::TYPE_LATEST_POST:				
				$msg = trans('messages.event_type_latest_post_decription', [
					'categories' => \Acelle\Library\Tool::getWPCategoriesPrase($this->getDataValue('categories')),
					'tags' => \Acelle\Library\Tool::getWPTagsPrase($this->getDataValue('tags'))
				]);
				break;
		}
    ...
    public function addCampaign()
    {
		...
		$campaign->user_id = $this->automation->user_id;
    ...
    {
        $zone = $this->automation->user->getTimezone();
    ...
    public function checkForSubscriberEvent()
    {
        ...
        if ($field == 'subscription_date') {
            $subscribers = Subscriber::select('subscribers.id')
                         ->whereRaw(sprintf('DATE(CONVERT_TZ(' . table('subscribers') . '.created_at, %1$s, %2$s) + %3$s) = DATE(CONVERT_TZ(NOW(), %1$s, %2$s)) AND DATE(CONVERT_TZ(' . table('subscribers') . '.created_at, %1$s, %2$s)) + %3$s <= CONVERT_TZ(NOW(), %1$s, %2$s)', db_quote(db_utc_offset()), db_quote(utc_offset($this->automation->user->getTimezone())), $this->getDelayIntervalMySql()))
                         ->whereRaw(sprintf('' . table('subscribers') . '.id NOT IN (SELECT COALESCE(subscriber_id, 0) FROM %3$s WHERE auto_event_id = %4$s AND EXTRACT(YEAR FROM CONVERT_TZ(NOW(), %1$s, %2$s)) = EXTRACT(YEAR FROM CONVERT_TZ(created_at, %1$s, %2$s)) )', db_quote(db_utc_offset()), db_quote(utc_offset($this->automation->user->getTimezone())), table('auto_triggers'), $this->id))
                         ->get();
        } else {
            $subscribers = Subscriber::select('subscribers.id')
                         ...
                         ->whereRaw(sprintf('DATE(CONVERT_TZ(' . table('subscriber_fields') . '.value, %1$s, %2$s) + %3$s) = DATE(CONVERT_TZ(NOW(), %1$s, %2$s)) AND DATE(CONVERT_TZ(' . table('subscriber_fields') . '.value, %1$s, %2$s)) + %3$s <= CONVERT_TZ(NOW(), %1$s, %2$s)', db_quote(db_utc_offset()), db_quote(utc_offset($this->automation->user->getTimezone())), $this->getDelayIntervalMySql()))
                         ->whereRaw(sprintf('' . table('subscribers') . '.id NOT IN (SELECT COALESCE(subscriber_id, 0) FROM %3$s WHERE auto_event_id = %4$s AND EXTRACT(YEAR FROM CONVERT_TZ(NOW(), %1$s, %2$s)) = EXTRACT(YEAR FROM CONVERT_TZ(created_at, %1$s, %2$s)) )', db_quote(db_utc_offset()), db_quote(utc_offset($this->automation->user->getTimezone())), table('auto_triggers'), $this->id))
                         ->get();
        }
    ...
    public function matchMonth()
    {
        ...
        return in_array(Carbon::now($this->automation->user->getTimezone())->month, $months, false);
    }
    ...
    public function matchDayOfMonth()
    {
        ...
        return in_array(Carbon::now($this->automation->user->getTimezone())->day, $days, false);
    }
    ...
    public function matchDayOfWeek()
    {
        ...
        return in_array(Carbon::now($this->automation->user->getTimezone())->dayOfWeek, $weekdays, false);
    }
    ...
    public function matchWeekOfMonth()
    {
        ...
        return in_array(Carbon::now($this->automation->user->getTimezone())->weekOfMonth, $weeks, false);
    }
    ...
    {        
		...
		//if($this->event_type == AutoEvent::TYPE_CUSTOM_CRITERIA) {
		//	$criteria = $this->getDataValue('criteria');
		//	
		//	// for custom cateria
		//	$conditions = array();
		...
		//			case AutoEvent::OPERATOR_EQUAL:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) = '".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_NOT_EQUAL:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) != '".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_CONTAINS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) LIKE '%".$keyword."%'";
		//				break;
		//			case AutoEvent::OPERATOR_NOT_CONTAINS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) NOT LIKE '%".$keyword."%'";
		//				break;
		//			case AutoEvent::OPERATOR_STARTS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) LIKE '".$keyword."%'";
		//				break;
		//			case AutoEvent::OPERATOR_ENDS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) LIKE '%".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_GREATOR:
		//				$cond = table('subscriber_fields') . ".value > '".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_LESS:
		//				$cond = table('subscriber_fields') . ".value < '".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_NOT_STARTS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) NOT LIKE '".$keyword."%'";
		//				break;
		//			case AutoEvent::OPERATOR_NOT_ENDS:
		//				$cond = 'LOWER(' . table('subscriber_fields') . ".value) NOT LIKE '%".$keyword."'";
		//				break;
		//			case AutoEvent::OPERATOR_NOT_BLANK:
		//				$cond = '(LOWER(' . table('subscriber_fields') . ".value) != '' AND LOWER(".table('subscriber_fields') . '.value) IS NOT NULL)';
		//				break;
		//			case AutoEvent::OPERATOR_BLANK:
		//				$cond = '(LOWER(' . table('subscriber_fields') . ".value) = '' OR LOWER(".table('subscriber_fields') . '.value) IS NULL)';
		//				break;
		//			default:
		//	
		//		}
		//	
		//		$conditions[] = \DB::getTablePrefix().'subscribers.id IN (SELECT ' . table('subscriber_fields') .
		//			'.subscriber_id FROM ' . table('subscriber_fields') .
		//			' LEFT JOIN ' . table('fields') . ' ON ' . table('subscriber_fields') . '.field_id = ' . table('fields') . '.id' .
		//			' WHERE (' . table('fields') .
		//			'.uid = "'.$condition['field_uid'] . '" AND '.$cond.'))';
		//	}
		//	
		//	//return $conditions;
